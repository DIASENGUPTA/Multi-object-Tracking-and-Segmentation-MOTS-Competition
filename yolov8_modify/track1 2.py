...
def run(
    ...
    for frame_idx, batch in enumerate(dataset):
        ...
        #with dt[2]:
            ...
            #print(im.cpu().numpy()[0].shape)
            #im_new=np.transpose(im.cpu().numpy()[0], (1, 2, 0))
            #print(im_new.shape)
...
#tensor_list.append(torch.zeros((100, 6)))
            ...
            p=[torch.Tensor(tensor_list).to(device)]
            #print(p.to(device))
        ...
        for i, det in enumerate(p):  # detections per image
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if(cls==0):
                            ...
                            filenamefinalco=open('/projectnb/rlvn/students/ksg25/maskdata/'+path_npy.split('/')[-1]+'_new.npy',"ba+")
            ...
            if save_vid:
                if vid_path[i] != save_path:  # new video
                    ...
                    #image = img[:, :, ::-1]  
                print(type(im0))
                visualizer = Visualizer(im0, metadata, instance_mode=ColorMode.IMAGE)
                pred_new=predictor(im0)
                if 'instances' in pred_new and len(pred_new['instances']) > 0:
                    scores = pred_new['instances'].scores
                    keep = torch.nonzero(scores >= 0.4).squeeze(1)
                    instances = pred_new['instances'][keep].to('cpu')
                #print(instances[0].pred_masks.shape)
                #imnew = visualizer.draw_instance_predictions(predictions=instances)
                #print(type(imnew))
                #imfinal=im.get_image().astype("uint8")        
    ...
    with open("scoring_metric_MOT.pkl","wb") as file: ## scoring metric
