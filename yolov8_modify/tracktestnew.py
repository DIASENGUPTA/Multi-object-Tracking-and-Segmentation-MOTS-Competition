            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        model = torch.load('/projectnb/rlvn/students/ksg25/projmod1.pth')#evaluated the model
                        print(img2.unsqueeze(0).shape)
                        print(torch.tensor(arr1.reshape(1,2)).shape)
                        output = model.forward(img2.unsqueeze(0),torch.Tensor(arr1.reshape(1,2)))#action class output
